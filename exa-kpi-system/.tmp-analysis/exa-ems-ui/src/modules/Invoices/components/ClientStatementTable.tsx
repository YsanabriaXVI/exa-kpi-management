...
const ClientStatementTable: React.FC<ClientStatementTableProps> = ({ data, loading, notes, showNotes, clientName, onSelectionChange }) => {
  ...
  return (
    <div className="client-statement-table">
      {renderHeaderSection()}
      ...
      <div className="table-responsive mb-3">
        <CTable hover striped bordered responsive className="mb-0 border bg-body" style={{ fontSize: '0.85rem', whiteSpace: 'nowrap' }}>
          ...
          <CTableBody>
            ...
            {/* Totals Row - Only show on last page or always? Usually totals are for the whole invoice, so show at bottom */}
            <CTableRow className="fw-bold">
              <CTableDataCell colSpan={2}></CTableDataCell>
              {/* We need to align totals with columns. The API returns a simple array of totals. 
                  We assume the order matches the columns, but we might need to skip the first few if they are not numeric.
                  However, the legacy code just mapped them. Let's try to map them to columns if possible, or just display them.
                  The legacy code mapped total array to columns.
              */}
              {total.map((val, index) => (
                 // Adjust index because we added Checkbox and # columns
                 // Actually, the total array length should match columns length?
                 // Let's check legacy logic: total = total.length ? total : new Array(this.state.columns.length);
                 // It maps total to columns.
...
export default ClientStatementTable
