...
import {
  ...
  fetchFuelTypes,
  resetStatuses,
  selectFuelTypeErrors,
  selectFuelTypeStatuses,
  selectFuelTypeList,
  selectFuelTypeLoadingList,
} from '../store/fuelType.slice'

import { permissionService, CREATE, UPDATE, DELETE } from '../../../../services/auth/permission.service'
import { MODULE_FUEL_TYPE } from '../../../../constants/modules'

const COLUMN_STORAGE_KEY = 'fuelTypes_visible_columns_v2'
...
const loadSavedColumns = () => {
  ...
  return ['fuelTypeId', 'name', 'updatedateFormat', 'createdateFormat', 'createdByName', 'updatedByName']
}

const FuelTypeListPage: React.FC = () => {
  const dispatch = useDispatch<AppDispatch>()
  const navigate = useNavigate()

  const list = useSelector(selectFuelTypeList)
  const errors = useSelector(selectFuelTypeErrors)
  const statuses = useSelector(selectFuelTypeStatuses)
  const loading = useSelector(selectFuelTypeLoadingList)
  ...
  const canCreate = permissionService.checkPermission(MODULE_FUEL_TYPE, CREATE)
  const canUpdate = permissionService.checkPermission(MODULE_FUEL_TYPE, UPDATE)
  const canDelete = permissionService.checkPermission(MODULE_FUEL_TYPE, DELETE)
  ...
  useEffect(() => {
    ...
    const msg = typeof errors === 'string' ? errors : 'An error occurred loading Fuel Types'
    toast?.error ? toast.error('Error', msg) : console.error(msg)
  }, [errors])

  useEffect(() => {
    const toast = (window as any).exaToast
    if (statuses.added) toast?.success?.('Success', 'Fuel Type was Added')
    if (statuses.updated) toast?.success?.('Success', 'Fuel Type was Updated')
    if (statuses.deleted) toast?.success?.('Success', 'Fuel Type was Deleted')
  ...
  const handleOpenNew = () => canCreate && navigate('/fuel/settings/fuel-types/new')
  const handleOpenEdit = (id: number) => canUpdate && navigate(`/fuel/settings/fuel-types/${id}`)
  ...
  const confirmDelete = async () => {
    ...
    await dispatch(deleteFuelType(pendingDeleteId))
  }
  ...
  const filteredItems = useMemo(() => {
    ...
    return tableItems.filter((x: any) => {
      const haystack = [x.fuelTypeId, x.name, x.createdateFormat, x.updatedateFormat, x.createdByName, x.updatedByName]
  ...
  const columns = [
    { key: 'fuelTypeId', label: 'ID', sorter: true, filter: true },
    { key: 'name', label: 'Name', sorter: true, filter: true },
    { key: 'updatedateFormat', label: 'Last Update', sorter: true, filter: true },
    { key: 'createdateFormat', label: 'Create Date', sorter: true, filter: true },
        ...
        <CCardBody>
          {errors && (
            <CAlert color="danger" className="mb-3">
              {typeof errors === 'string' ? errors : 'An error occurred loading Fuel Types'}
            </CAlert>
          )}
                ...
                <td>
                  {canUpdate && (
                    <CButton
                      ...
                      variant="ghost"
                      size="sm"
                      onClick={() => handleOpenEdit(item.fuelTypeId)}
                      title="Edit"
                    >
                  ...
                  {canDelete && (
                    <CButton
                      ...
                      variant="ghost"
                      size="sm"
                      onClick={() => handleAskDelete(item.fuelTypeId)}
                      title="Delete"
                    >
                      <CIcon icon={cilTrash} />
                    </CButton>
