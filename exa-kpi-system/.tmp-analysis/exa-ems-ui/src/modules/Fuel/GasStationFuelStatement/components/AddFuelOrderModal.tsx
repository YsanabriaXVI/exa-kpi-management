...
const AddFuelOrderModal: React.FC<AddFuelOrderModalProps> = ({
  ...
  const handleAdd = () => {
    onAdd(invoiceLines.filter((r) => selectedIds.has(r.id)))
  ...
  return (
    <CModal visible={visible} onClose={onClose} size="xl" scrollable>
      <CModalHeader><CModalTitle>Add Fuel Orders</CModalTitle></CModalHeader>
      <CModalBody>
        {invoiceLines.length === 0 ? (
          <p className="text-muted">No additional invoice lines available.</p>
        ) : (
          <CSmartTable items={invoiceLines} columns={COLUMNS} itemsPerPage={15} pagination
      ...
      <CModalFooter>
        <CButton color="secondary" variant="outline" onClick={() => {
          selectedIds.size === invoiceLines.length ? setSelectedIds(new Set()) : setSelectedIds(new Set(invoiceLines.map((r) => r.id)))
        }}>
