...
import {
  ...
  fetchFuelUnitTypes,
  resetStatuses,
  selectFuelUnitTypeErrors,
  selectFuelUnitTypeStatuses,
  selectFuelUnitTypeList,
  selectFuelUnitTypeLoadingList,
...
const COLUMN_STORAGE_KEY = 'fuelUnitTypes_visible_columns'
...
const loadSavedColumns = () => {
  ...
  return ['name', 'updateDateFormat', 'createDateFormat', 'createdByName', 'updatedByName']
}

const FuelUnitTypeListPage: React.FC = () => {
  const dispatch = useDispatch<AppDispatch>()
  const navigate = useNavigate()

  const list = useSelector(selectFuelUnitTypeList)
  const errors = useSelector(selectFuelUnitTypeErrors)
  const statuses = useSelector(selectFuelUnitTypeStatuses)
  const loading = useSelector(selectFuelUnitTypeLoadingList)
  ...
  const canDelete = permissionService.checkPermission(MODULE_FUEL_UNIT_TYPE, DELETE)

  useEffect(() => {
    dispatch(fetchFuelUnitTypes())
  }, [dispatch])
  ...
  useEffect(() => {
    ...
    const msg = typeof errors === 'string' ? errors : 'An error occurred loading Fuel Unit Types'
    toast?.error ? toast.error('Error', msg) : console.error(msg)
  }, [errors])

  useEffect(() => {
    const toast = (window as any).exaToast
    if (statuses.added) toast?.success?.('Success', 'Fuel Unit Type was Added')
    if (statuses.updated) toast?.success?.('Success', 'Fuel Unit Type was Updated')
    if (statuses.deleted) toast?.success?.('Success', 'Fuel Unit Type was Deleted')
  ...
  const handleOpenNew = () => canCreate && navigate('/fuel/settings/fuel-unit-types/new')
  const handleOpenEdit = (id: number) => canUpdate && navigate(`/fuel/settings/fuel-unit-types/${id}`)
  ...
  const confirmDelete = async () => {
    ...
    await dispatch(deleteFuelUnitType(pendingDeleteId))
  }
  ...
  const filteredItems = useMemo(() => {
    ...
    return tableItems.filter((x: any) => {
      const haystack = [x.name, x.createdByName, x.updatedByName, x.createDateFormat, x.updateDateFormat]
  ...
  return (
    <CContainer fluid>
        ...
        <CCardBody>
          {errors && (
            <CAlert color="danger" className="mb-3">
              {typeof errors === 'string' ? errors : 'An error occurred loading Fuel Unit Types'}
            </CAlert>
          )}
                  ...
                  <td>
                    {canUpdate && (
                      <CButton
                        ...
                        variant="ghost"
                        size="sm"
                        onClick={() => handleOpenEdit(item.unitTypeId)}
                        title="Edit"
                      >
                    ...
                    {canDelete && (
                      <CButton
                        ...
                        variant="ghost"
                        size="sm"
                        onClick={() => handleAskDelete(item.unitTypeId)}
                        title="Delete"
                      >
                        <CIcon icon={cilTrash} />
                      </CButton>
      ...
      <ConfirmDialog
        ...
        message="Are you sure you want to delete this fuel unit type?"
        onClose={() => {
...
export default FuelUnitTypeListPage
