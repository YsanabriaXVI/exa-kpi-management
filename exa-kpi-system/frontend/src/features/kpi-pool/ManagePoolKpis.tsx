...
const availabilityCopy: Record<PoolKpiAvailability, string> = {
  ...
  IN_POOL: "Included in this Period",
...
export function ManagePoolKpis() {
  ...
  const formatPoolPeriod = usePoolPeriodFormatter(poolId);
  ...
  const targetPeriodLabel = targetPeriod ? formatPoolPeriod(targetPeriod) : "selected period";
  const catalogQuery = useQuery({ queryKey: ["pool-manage-kpis", poolId, targetPeriod, periodIsEditable], queryFn: () => periodIsEditable ? kpiPoolService.getManageableKpis(poolId, targetPeriod) : kpiPoolService.getManageableComposition(poolId, targetPeriod), enabled: poolId > 0 && Boolean(targetPeriod) && Boolean(editingPeriod) && editingPeriod?.workflowStatus !== "FUTURE", retry: false });
  ...
  const refresh = async () => {
    ...
    await Promise.all([
      ...
      queryClient.invalidateQueries({ queryKey: ["scorecard-periods"] }),
      queryClient.invalidateQueries({ queryKey: ["scorecard-available"] }),
    ]);
  };
  const addMutation = useMutation({
    ...
    onSuccess: async (_, codes) => { setActiveCard("IN_POOL"); setPage(1); setNoticeTone("success"); setNotice(codes.length === 1 ? `${codes[0]} was linked to this Pool.` : `${codes.length} KPI Configurations were linked to this Pool.`); await refresh(); },
    onError: (mutationError) => { setNoticeTone("warning"); setNotice(mutationError instanceof Error ? mutationError.message : "The selected KPI Configurations could not be linked."); },
  });
  ...
  const finalizeMutation = useMutation({
    ...
    onSuccess: async (result) => { setFinalizeConfirmationOpen(false); setNoticeTone("success"); setNotice(`${formatPoolPeriod(result.data.periodStart)} composition was finalized with ${result.data.kpiCount} KPI ${result.data.kpiCount === 1 ? "Configuration" : "Configurations"}.`); await refresh(); },
  ...
  const records = useMemo(() => {
    ...
    const composition = effectiveCompositionQuery.data;
    if (!composition) return catalog;
    const includedIds = new Set(composition.map(item => item.configurationId));
    const catalogIds = new Set(catalog.map(item => item.configurationId));
    return [
      ...catalog.map(item => includedIds.has(item.configurationId) ? { ...item, availability: "IN_POOL" as const } : item),
      ...composition.filter(item => !catalogIds.has(item.configurationId)).map(item => ({ ...item, availability: "IN_POOL" as const, reasonCode: null, conflictingConfigurationCode: null })),
    ];
  }, [catalogQuery.data, effectiveCompositionQuery.data]);
  ...
  useEffect(() => { if (!editingPeriod) return; setActiveCard(editingPeriod.workflowStatus === "FINALIZED" || params.get("view") === "included" && editingPeriod.start === requestedPeriod ? "IN_POOL" : "AVAILABLE"); setPage(1); setSelected([]); setHiddenFromView([]); }, [poolId, editingPeriod?.start, editingPeriod?.workflowStatus, requestedPeriod, params]);
  const counts = {
  ...
  const toggle = (code: string) => setSelected((current) => {
    ...
    if (candidate.availability === "NOT_AVAILABLE") return current;
    return [...current, code];
  });
  ...
  return (
    <main className="pool-page manage-kpis-page">
      ...
      {poolQuery.data && <section className="manage-compact-period-context" aria-label="Composition period context">
        <div className="manage-period-selector-block"><span>Select Pool Period</span><PoolPeriodSelect periods={periodsQuery.data?.data ?? []} value={targetPeriod} onChange={(period) => { setTargetPeriod(period); setSelected([]); }}/><div className="manage-period-guidance">{isFinalized ? <small className="finalized-period-note"><Check size={15} aria-hidden="true"/><span><strong>{formatPoolPeriod(targetPeriod)} finalized</strong> — {records.length} KPI {records.length === 1 ? "Configuration is" : "Configurations are"} available to Scorecards. This composition is read-only.</span></small> : isFirstInputPeriod && periodIsEditable ? <small className="first-period-note"><CheckCircle2 size={15} aria-hidden="true"/><span><strong>This is the first Input Period of the Pool.</strong> The composition can be finalized once you have selected the KPI Configurations that should be available to Scorecards.</span></small> : periodCanFinalize ? <small><strong>Editable composition.</strong> You can add, remove or replace KPI Configurations for this period.</small> : periodIsEditable ? <><small><strong>You may continue preparing this composition.</strong></small><small className="finalization-warning"><TriangleAlert size={15} aria-hidden="true"/> <span>Finalization will become available after {editingPeriod?.dependency.previousPeriodStart ? formatPoolPeriod(editingPeriod.dependency.previousPeriodStart) : "the previous period"} Monitoring is closed.</span></small></> : <small>This composition is not available yet.</small>}</div></div>
      ...
      {isFinalized && periodsQuery.data?.meta.defaultPeriodStart && <button type="button" className="button secondary" onClick={() => setTargetPeriod(periodsQuery.data!.meta.defaultPeriodStart!)}>Prepare next period: {formatPoolPeriod(periodsQuery.data.meta.defaultPeriodStart)}</button>}

      {editingPeriod?.workflowStatus === "FUTURE" && <section className="future-composition-empty"><Hourglass size={34} /><span className="selected-period-badge">{formatPoolPeriod(targetPeriod)} · Future</span><h2>KPI composition not available yet</h2><p>Complete the previous period workflow before this composition becomes available.</p><strong>Current editable composition: {formatPoolPeriod(periodsQuery.data?.meta.defaultPeriodStart ?? targetPeriod)}</strong><button className="button secondary" onClick={() => setTargetPeriod(periodsQuery.data?.meta.defaultPeriodStart ?? targetPeriod)}>Back to editable period</button></section>}

      {editingPeriod?.workflowStatus !== "FUTURE" && <><section className="selection-summary">
        <div className="manage-section-heading period-section-heading"><div><h2>Selection Summary for {targetPeriod ? formatPoolPeriod(targetPeriod) : "Selected Period"}</h2><p>Available to Add lists eligible configurations with the Pool frequency. Select and add them to include them in this period; Scorecards can use the included set after finalization.</p></div></div>
        <div className="availability-cards">
          ...
          <AvailabilityCard type="IN_POOL" label="Included in this Period" count={counts.IN_POOL} active={activeCard === "IN_POOL"} onClick={() => setActiveCard(activeCard === "IN_POOL" ? "ALL" : "IN_POOL")} />
        ...
        <div className="pool-inner-table manage-kpi-table-wrap stable-table-shell">
          <table className={`kpi-table manage-kpi-table ${isFinalized ? "snapshot-table" : "editable-table"}`}>
            <thead><tr>
              ...
              <SortableTableHeader active={sort.key === "kpiCode"} direction={sort.direction} onSort={() => sortBy("kpiCode")}>Definition</SortableTableHeader>
              <SortableTableHeader active={sort.key === "name"} direction={sort.direction} onSort={() => sortBy("name")}>KPI Configuration</SortableTableHeader>
            ...
            <tbody key={filterAnimationKey}>{catalogQuery.isLoading ? <tr><td colSpan={isFinalized ? 9 : 11} className="table-message">Loading KPI Configurations...</td></tr> : paginated.length ? paginated.map((record) => (
              <tr key={record.configCode} className={selected.includes(record.configCode) ? "selected-row" : ""}>
                {!isFinalized && <><td><input type="checkbox" disabled={!periodIsEditable || record.availability === "NOT_AVAILABLE"} checked={selected.includes(record.configCode)} onChange={() => toggle(record.configCode)} aria-label={`Select ${record.configCode}`} title={availabilityReason(record)} /></td><td><span title={availabilityReason(record)} className={`availability-label ${record.availability.toLowerCase()}`}>{availabilityCopy[record.availability]}</span></td></>}
                <td><span className="code-pill">{record.configCode}</span></td><td><strong>{record.sourceDefinitionName ?? record.kpiCode}</strong><small>{record.sourceDefinitionName ? record.kpiCode : ""}</small></td><td className="name-cell"><strong>{record.name}</strong>{record.subjectLabel && <small>Subject: {record.subjectLabel}</small>}</td><td>{record.category}</td><td>{record.evaluationScope === "BY_SUBJECT" ? <EntityGoalsSummary entityEvaluationMode={record.entityEvaluationMode} count={record.subjectGoalCount ?? 0} groupGoal={record.groupGoal} onView={record.configurationId ? () => setEntityGoalsId(record.configurationId!) : undefined}/> : record.goal}</td><td>{record.measurementUnit}</td><td>{record.dataSource}</td>
                <td><span className={`status-chip ${record.status.toLowerCase()}`}><i />{record.status === "ACTIVE" ? "Active" : "Inactive"}</span></td>
                <td><div className="table-actions">
                  {periodIsEditable && record.availability === "AVAILABLE" && <button type="button" className="icon-button add" title={"Include " + record.configCode + " in this period"} aria-label={"Add " + record.configCode + " to this period"} disabled={addMutation.isPending} onClick={() => addMutation.mutate([record.configCode])}><Plus size={15}/></button>}
                  ...
                  {record.availability === "IN_POOL" && record.configurationId && editingPeriod?.poolPeriodId && scorecardUsageQuery.data?.assignments.find((item)=>item.kpiConfigurationId===record.configurationId)?.scorecardCompositionStatus !== "FINALIZED" && <button className="icon-button edit" title={`Edit ${record.configCode} only for ${formatPoolPeriod(targetPeriod)}`} aria-label={`Edit ${record.configCode} for this Pool period`} onClick={() => navigate(`/app/kpi-management/config/set?mode=POOL_PERIOD_EDIT&kpiConfigId=${encodeURIComponent(record.configurationId!)}&poolId=${poolId}&inputPeriodId=${editingPeriod.poolPeriodId}&period=${encodeURIComponent(targetPeriod)}&from=pool-manage`)}><Pencil size={15}/></button>}
                </div></td>
              </tr>
