...
export function KpiPoolOverview() {
  ...
  const apiSortBy: Record<PoolSortKey, string> = { code: "poolCode", name: "poolName", companies: "poolCode", frequency: "inputFrequencyCode", validity: "validFrom", operationalPeriod: "poolCode", kpis: "poolCode", scorecards: "poolCode", status: "statusCode" };
      ...
      <div className="kpi-table-wrap pool-table-wrap stable-table-shell">
        <table className="kpi-table pool-table">
          <thead><tr>
            ...
            <SortableTableHeader active={sort.key === "frequency"} direction={sort.direction} onSort={() => sortBy("frequency")}><>Input<br />Frequency</></SortableTableHeader>
            ...
            <SortableTableHeader active={sort.key === "operationalPeriod"} direction={sort.direction} onSort={() => sortBy("operationalPeriod")}>Operational Period</SortableTableHeader>
            <SortableTableHeader active={sort.key === "kpis"} direction={sort.direction} onSort={() => sortBy("kpis")}>KPIs Available</SortableTableHeader>
            <SortableTableHeader active={sort.key === "scorecards"} direction={sort.direction} onSort={() => sortBy("scorecards")}>Scorecards Using</SortableTableHeader>
          ...
          <tbody>{poolsQuery.isLoading ? <tr><td colSpan={10} className="table-message">Loading KPI Pools...</td></tr> : visiblePools.length ? visiblePools.map((pool) => (
            <tr key={pool.id}>
              ...
              <td><PeriodCell period={pool.operationalPeriod}/></td>
              <td className="pool-number">{pool.operationalPeriod?.status === "FINALIZED" ? pool.operationalPeriod.kpiCount ?? "—" : "—"}</td>
              <td className="pool-number">{pool.operationalPeriod?.status === "FINALIZED" ? usageQuery.isLoading ? "…" : usageQuery.isError ? "—" : usageByPeriod.get(`${pool.id}:${pool.operationalPeriod.periodKey}`) ?? 0 : "—"}</td>
...
type PoolSortKey = "code" | "name" | "companies" | "frequency" | "validity" | "operationalPeriod" | "kpis" | "scorecards" | "status";
