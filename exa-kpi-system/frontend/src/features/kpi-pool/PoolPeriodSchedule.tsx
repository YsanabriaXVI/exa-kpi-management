...
export function PoolPeriodSchedule() {
  ...
  return <main className="pool-page pool-period-schedule-page">
    ...
    <SelectedPeriodStatusCard periods={periods} period={selected} editable={editable} onManage={() => navigate(`/app/pool-kpis/manage-kpis?poolId=${poolId}&period=${encodeURIComponent(selected.start)}`)}/>

    <section className="input-period-workflow" aria-labelledby="input-period-workflow-title"><header><h2 id="input-period-workflow-title">Input Period Workflow Status — {periodDisplay(selected, periods).label}</h2><p>Track how this Input Period moves through Pool Composition, Scorecard KPI Selection and Monitoring Results.</p></header><div className={`period-workflow-board ${selected.workflowStatus.toLowerCase()}`}>
      <StageCard step={1} title="Pool Composition" state={poolStageState(selected)} icon={<Layers3 size={20}/>}>
        {selected.workflowStatus === "FINALIZED" ? <><strong>{compositionCount} KPI {compositionCount === 1 ? "Configuration" : "Configurations"}</strong><p>Read-only set available to Scorecards for this Input Period.</p><button className="button secondary" onClick={() => navigate(`/app/pool-kpis/detail/${poolId}?period=${selected.start}`)}>View Composition</button></> : selected.workflowStatus === "EDITABLE" ? <><strong>You may continue preparing this composition.</strong><p>{compositionCount} KPI {compositionCount === 1 ? "Configuration is" : "Configurations are"} currently included. {selected.canFinalizeComposition ? "The composition can be finalized when ready." : finalizationReason(selected, periods)}</p><button className="button primary" onClick={() => navigate(`/app/pool-kpis/manage-kpis?poolId=${poolId}&period=${encodeURIComponent(selected.start)}`)}>Manage Composition</button></> : <><strong>Not available yet</strong><p>This stage is not available for this Input Period yet.</p></>}
      </StageCard>
...
function SelectedPeriodStatusCard({ period, periods, editable, onManage }: { periods: PoolInputPeriod[]; period: PoolInputPeriod; editable?: PoolInputPeriod; onManage: () => void }) {
  const periodLabel = periodDisplay(period, periods).label;
  ...
  return <section className="selected-period-status-card preparing" aria-live="polite"><span className="selected-period-status-icon"><PencilLine size={24}/></span><div className="selected-period-status-copy"><header><span>{periodLabel}</span><b>Preparing</b></header><h2>Pool Composition in Preparation</h2><p>You may add, remove or replace KPI Configurations for this Input Period.</p>{period.dependency.previousPeriodStart && <small className="selected-period-dependency"><TriangleAlert size={14}/> Finalization is waiting for {periodLabelAt(period.dependency.previousPeriodStart, periods)} Monitoring to close.</small>}<button className="button primary" onClick={onManage}>Manage {periodLabel} KPIs</button></div></section>;
}
...
function finalizationReason(period: PoolInputPeriod, periods: PoolInputPeriod[]) { return period.dependency.previousPeriodStart ? `May be prepared now. Waiting for ${periodLabelAt(period.dependency.previousPeriodStart, periods)} Monitoring to close before finalization.` : "May be prepared now."; }
...
function periodLabelAt(start: string, periods: PoolInputPeriod[]) { const period = periods.find(item => item.start === start); return period ? periodDisplay(period, periods).label : formatPeriod(start); }
