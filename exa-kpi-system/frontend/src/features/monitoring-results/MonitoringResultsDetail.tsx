...
export function MonitoringResultsDetail() {
  ...
  return <main className="monitor-page monitor-detail-page">
    ...
    <header className="monitor-header"><div><span className="monitor-eyebrow">HISTORICAL PERIOD PROJECTION</span><h1>Monitoring Detail</h1><p>Server-side KPI result detail for one immutable Monitoring Period identity.</p></div></header>
    {!id ? <section className="result-entry-live-state"><p>Select a Monitoring Period from Overview.</p></section> : query.isLoading ? <section className="result-entry-live-state"><p>Loading Monitoring Detail…</p></section> : query.isError ? <section className="result-entry-live-state"><p>Monitoring Detail could not be loaded.</p><button onClick={() => query.refetch()}>Retry</button></section> : <>
      ...
      <section className="result-entry-live-card"><header><div><h2>KPI Results</h2><p>Filtering, sorting and pagination are executed by Monitoring.</p></div></header>
