...
function PoolCalendar({ pool, destination }: { pool: { id: number; code: string; name: string }; destination: "result-entry" | "detail" }) {
  ...
  return <article className="pool-calendar"><header><div><small>{pool.code}</small><h3>{pool.name}</h3></div><Link to={`/app/monitoring-results/pool-input-schedule?poolId=${pool.id}`}>Full schedule</Link></header>
    ...
    }}><strong>{period.periodKey}</strong><span>{period.monitoringPeriod ? period.status.replaceAll("_", " ") : period.poolWorkflowStatus === "FINALIZED" ? "Check availability" : period.poolWorkflowStatus === "FUTURE" ? "Future period" : "Awaiting Pool finalization"}</span><small>{period.monitoringPeriod ? `${period.entered}/${period.expected} Results` : "Not initialized"}</small></button>)}</div>
    {schedule.data && !schedule.data.data.length && <p>No Input Periods configured.</p>}
    {selected && <div className="pool-calendar-readiness" aria-live="polite">{readiness.isFetching ? <p>Checking Pool and Scorecard readiness…</p> : readiness.isError ? <p role="alert">{readiness.error.message}</p> : resolved && <><strong>{resolved.inputPeriod?.periodKey}</strong><p>{resolved.reason ?? "Monitoring is available for this period."}</p>
