export type CheckEvaluation = {
  historical?: {referenceType:string;comparisonDirection:string;signedChangePercent:string|null;achievedChangePercent:string|null;resolution?:{value:string;sourceType:string}|null;requiredPeriod?:{key:string}|null}|null;
  ...
  rawAchievementPercent:string|null;compliancePercent:string|null;goalMet:boolean|null;trafficLight:string|null;
...
export function CheckResultsReview({report,dirty,busy,readOnly,selected,onCheck}:{report?:CheckReport;dirty:boolean;busy:boolean;readOnly:boolean;selected:boolean;onCheck:()=>void}) {
  ...
  return <section className="check-results-review" aria-label="Check Results review">
    ...
    <p role="status">{dirty?"Unsaved Results. Save Results before checking.":report?.status==="STALE"?"Results or baseline changed. Run Check Results again.":current?`CURRENT · Check #${report.runNo} · Results v${report.basedOnResultsVersion}${report.basedOnBaselineVersion===undefined?"":` · Baselines v${report.basedOnBaselineVersion}`}`:"NOT_CHECKED"}</p>
    ...
    {current&&report.summary&&<>
      ...
      <div className="manual-entry-table-wrap"><table className="manual-entry-table check-evaluations"><thead><tr><th>KPI / Entity</th><th>Goal</th><th>Result</th><th>Raw Achievement</th><th>Score / Compliance</th><th>Goal Met</th><th>Traffic</th><th>Weight</th><th>Weighted Contribution</th><th>Evaluation</th></tr></thead><tbody>
        {report.evaluations.map(row=><tr key={row.id}><td>{row.kpiName}{row.entityLabel&&<small>{row.entityLabel}</small>}</td><td>{row.goal??"—"} {row.goalUnit??row.unit}</td><td>{row.resultValue??"—"} {row.unit}</td><td>{percent(row.rawAchievementPercent)}</td><td>{percent(row.compliancePercent)}</td><td>{row.goalMet===null?"—":row.goalMet?"Yes":"No"}</td><td><TrafficLight value={row.trafficLight}/></td><td>{percent(row.weight)}</td><td>{percent(row.weightedContribution)}</td><td>{row.status}{row.historical&&<small>Baseline: {row.historical.resolution?.value??"Missing"} {row.unit} · {row.historical.requiredPeriod?.key??"—"}<br/>Actual change: {percent(row.historical.signedChangePercent)} · Achieved change: {percent(row.historical.achievedChangePercent)}</small>}</td></tr>)}
      </tbody></table></div>
      ...
      <h3>Findings</h3>
      {!report.findings.length?<p>No findings.</p>:<ul className="check-findings">{report.findings.map((finding,index)=><li key={`${finding.code}-${index}`}><strong>{finding.code}</strong> · {finding.severity} · {finding.blocking?"Blocks future Submit":"Non-blocking"}<p>{finding.kpiCode??report.scorecards.find(c=>c.id===finding.scorecardId)?.name}{finding.entityLabel?` · ${finding.entityLabel}`:""}</p><p>{finding.message}</p></li>)}</ul>}
    </>}
  </section>;
}
