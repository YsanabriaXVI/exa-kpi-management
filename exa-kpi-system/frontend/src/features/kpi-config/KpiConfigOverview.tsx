...
export function KpiConfigOverview() {
  ...
  const [actionToast, setActionToast] = useState<{ message: string; tone: "success" | "info" } | null>(() => params.get("created") ? { message: "KPI Configuration created successfully. It is now ready to be added to a KPI Pool.", tone: "success" } : null);
  ...
  const deactivateMutation = useMutation({
    mutationFn: kpiConfigService.deactivate,
    onSuccess: () => {
  ...
  const filtered = useMemo(() => configurations.filter((config) => {
    ...
    return (!term || `${config.code} ${config.definitionCode} ${config.definitionName}`.toLowerCase().includes(term)) &&
  ...
  return (
    <main className="kpi-config-page">
      ...
      {actionToast && <ActionToast message={actionToast.message} tone={actionToast.tone} onClose={() => setActionToast(null)} />}
      <section className="config-overview-toolbar">
        <label className="config-search"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search code, KPI name or description..." /></label>
        <ConfigMultiSelect label="All KPIs" options={kpiOptions} selected={selectedKpis} onChange={setSelectedKpis} searchable />
      ...
      {selectedConfigurations.length > 0 && <section className="config-bulk-bar" aria-live="polite">
        ...
        <div>
          ...
          <button type="button" className="button config-send-pool-button" onClick={() => setSendModalOpen(true)}><Send size={15} /> Send KPIs to a Pool</button>
        </div>
      </section>}
