...
export function ConfigMultiSelect({
  ...
  return (
    <div className="config-multiselect" ref={rootRef}>
      <button
        type="button"
        className="config-filter-trigger"
        aria-expanded={open}
        onClick={() => setOpen((value) => !value)}
      >
        {selectedOptions.length ? (
          <span
            className={`config-filter-chips ${hiddenCount > 0 ? "has-more" : ""}`}
          >
          ...
          <span className="config-filter-placeholder">{label}</span>
        )}
        <ChevronDown size={14} />
      </button>
      {open && (
        <div className="config-filter-menu">
          {searchable && (
            <label className="config-filter-search">
              <Search size={13} />
              <input
                autoFocus
                value={search}
          ...
          {showClearOption && <button
            ...
            onClick={() => onChange([])}
          >
            <i className={selected.length === 0 ? "checked" : ""}>
          ...
          <div className="config-filter-option-list">
            ...
            {!filtered.length && (
              <p className="config-filter-empty">{emptyText}</p>
            )}
          </div>
        </div>
      )}
