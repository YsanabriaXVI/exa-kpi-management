...
export function ScorecardAssignment() {
  ...
  const kpiWeightTotal = kpis.reduce((sum, item) => sum + (item.evaluationScope === "BY_SUBJECT" && item.evaluations?.length ? item.evaluations.reduce((subtotal, entity) => subtotal + (entity.weight ?? 0), 0) : item.weight), 0);
