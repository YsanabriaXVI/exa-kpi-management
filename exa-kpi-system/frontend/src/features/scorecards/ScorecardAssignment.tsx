...
export function ScorecardAssignment() {
  ...
  const [periodSelection, setPeriodSelection] = useState({ scorecardId, key: requestedPeriod });
  const periodKey = periodSelection.scorecardId === scorecardId ? periodSelection.key : requestedPeriod;
  const setPeriodKey = (key: string) => setPeriodSelection({ scorecardId, key });
  ...
  const previousCompositionQuery = useQuery({ queryKey: ["scorecard-composition", scorecardId, previousPeriodKey], queryFn: () => scorecardService.composition(scorecardId, previousPeriodKey!), enabled: scorecardId > 0 && Boolean(previousPeriodKey) && !!compositionQuery.data, retry: false });
  ...
  useEffect(() => { if (periodsQuery.data?.length && !periodsQuery.data.some(period => period.periodKey === periodKey)) setPeriodKey(periodsQuery.data.find((period) => ["PREPARING", "AVAILABLE", "NOT_STARTED"].includes(period.scorecardCompositionStatus))?.periodKey ?? periodsQuery.data.find((period) => period.scorecardCompositionStatus !== "UNAVAILABLE")?.periodKey ?? periodsQuery.data[0].periodKey); }, [periodKey, periodsQuery.data]);
  useEffect(() => { if (!compositionQuery.data) { setKpis([]); setLinked([]); setSaved(false); return; } setKpis(compositionQuery.data.kpis.map((item) => ({ id: item.kpiConfigurationExternalId, configCode: item.configurationCode, code: item.definitionCode, name: item.definitionName, category: item.categoryName ?? "Not specified", goal: item.goal ?? "Not specified", measurementUnit: item.measurementUnit ?? "Not specified", source: item.dataSource ?? "Not specified", weight: Number(item.weight), evaluationScope: item.evaluationScope, entityEvaluationMode: item.entityEvaluationMode, groupGoal:item.groupGoal, goalUnit: item.goalUnit, resultUnit: item.resultUnit, evaluations: item.evaluations?.map(entity => ({...entity, weight: entity.weight === null ? null : Number(entity.weight)})) }))); setLinked(compositionQuery.data.linkedScorecards.map((item) => ({ id: item.linkedScorecardId, code: item.code, name: item.name, company: item.companies.join(", ") || "Not specified", department: item.departments.join(", ") || "Not specified", frequency: scorecardQuery.data?.inputFrequency ?? "Not available", weight: Number(item.weight) }))); setSaved(true); }, [compositionQuery.data, scorecardQuery.data?.inputFrequency]);
  useEffect(() => {
  ...
  const kpiWeightTotal = kpis.reduce((sum, item) => sum + (item.evaluationScope === "BY_SUBJECT" && item.entityEvaluationMode !== "CONTRIBUTE_TO_OVERALL" && item.evaluations?.length ? item.evaluations.reduce((subtotal, entity) => subtotal + (entity.weight ?? 0), 0) : item.weight), 0);
  ...
  const saveMutation = useMutation({ mutationFn: () => scorecardService.updateWeights(scorecardId, periodKey, { kpis: kpis.map((item) => ({ kpiConfigurationExternalId: item.id, weight: item.evaluationScope === "BY_SUBJECT" && item.entityEvaluationMode !== "CONTRIBUTE_TO_OVERALL" ? 0 : item.weight, entityWeights: item.evaluationScope === "BY_SUBJECT" && item.entityEvaluationMode !== "CONTRIBUTE_TO_OVERALL" ? item.evaluations?.map(({subjectExternalId, weight}) => ({subjectExternalId, weight})) : undefined })), linkedScorecards: linked.map((item) => ({ linkedScorecardId: item.id, weight: item.weight })) }), onSuccess: () => { setSaved(true); setSaveNotice(Date.now()); queryClient.invalidateQueries({ queryKey: ["scorecard-composition", scorecardId, periodKey] }); } });
  ...
  const compositionReadOnly = !compositionQuery.data || compositionQuery.data.status === "FINALIZED";
  ...
  const explainDisabledAction = (message: string) => {
    setFinalizeNotice(0);
    setSaveNotice(0);
  ...
  return <main className={`scorecard-page assignment-page ${selectorMode ? "assignment-data-loaded" : ""} ${readOnlyFromOverview ? "assignment-read-only" : ""} ${poolCompositionUnavailable ? "assignment-period-unavailable" : ""}`} key={scorecardId}>
    <nav className="kpi-breadcrumb"><Link to="/app/scorecards/overview">ScoreCards</Link><span>/</span><span>ScoreCard Assignment</span></nav>
    ...
    <section className="assignment-scorecard-search" aria-labelledby="assignment-scorecard-search-title"><div><h2 id="assignment-scorecard-search-title">Select ScoreCard</h2><p>Search by ScoreCard code, name or KPI Pool.</p></div>{scorecardSelector}</section>
    {(periodsQuery.isLoading || compositionQuery.isLoading) && <p role="status">Loading Input Period composition…</p>}
    {(periodsQuery.isError || compositionQuery.isError) && <p role="alert">{periodsQuery.error?.message ?? compositionQuery.error?.message}<button type="button" className="button secondary" onClick={() => { void periodsQuery.refetch(); void compositionQuery.refetch(); }}>Retry</button></p>}
    <section className="assignment-period-context" aria-label="ScoreCard composition context">
      <article className="assignment-period-card"><span><CalendarDays size={20}/></span><div><small>Input Period</small><div className="assignment-period-select-row"><span className="assignment-period-select-control"><select aria-label="Input Period" value={periodKey} onChange={(event) => setPeriodKey(event.target.value)}>{periodsQuery.data?.map((period) => <option key={period.periodKey} value={period.periodKey}>{formatPeriodKey(period.periodKey)}</option>)}</select><ChevronDown size={17} aria-hidden="true"/></span></div><p>Inherited from {scorecard.poolSource}</p></div></article>
    ...
    <section className="assignment-section">
      ...
      <div className="assignment-table-wrap"><table className="assignment-table"><thead><tr>{([...[{ key: "configCode", label: "KPI Config" }, { key: "name", label: "KPI" }, { key: "category", label: "KPI Category" }, { key: "goal", label: "Goal" }, { key: "measurementUnit", label: "Measurement Unit" }, { key: "source", label: "Data Source" }, { key: "weight", label: "Weight" }]] as Array<{ key: keyof AssignmentKpi; label: string }>).map((column) => <th key={column.key} aria-sort={kpiSort.key === column.key ? (kpiSort.direction === "asc" ? "ascending" : "descending") : "none"}><button type="button" className={kpiSort.key === column.key ? "assignment-sort-header active" : "assignment-sort-header"} onClick={() => toggleKpiSort(column.key)}>{column.label}<ArrowUpDown size={14}/></button></th>)}</tr></thead><tbody>
        {!sortedKpis.length && <tr><td colSpan={7} className="assignment-empty-cell">Select KPIs from this Pool to start the composition for this Input Period.</td></tr>}{sortedKpis.map((kpi) => <Fragment key={kpi.id}><tr className={kpi.evaluationScope === "BY_SUBJECT" && kpi.entityEvaluationMode !== "CONTRIBUTE_TO_OVERALL" ? "assignment-entity-parent" : undefined}><td><span className="code-pill">{kpi.configCode}</span></td><td><strong>{kpi.name}</strong><small>{kpi.code}</small>{kpi.entityEvaluationMode === "CONTRIBUTE_TO_OVERALL" && <small>By Entity / Contributes to Overall / SUM</small>}{kpi.evaluationScope === "BY_SUBJECT" && kpi.entityEvaluationMode !== "CONTRIBUTE_TO_OVERALL" && <EntityGoalsSummary count={kpi.evaluations?.length ?? 0} groupGoal={kpi.groupGoal}/>}</td><td>{kpi.category}</td><td>{kpi.evaluationScope === "BY_SUBJECT" && kpi.entityEvaluationMode !== "CONTRIBUTE_TO_OVERALL" ? "Entity targets" : kpi.goal}</td><td>{kpi.resultUnit ?? kpi.measurementUnit}</td><td>{kpi.source}</td><td>{kpi.evaluationScope === "BY_SUBJECT" && kpi.entityEvaluationMode !== "CONTRIBUTE_TO_OVERALL" && kpi.evaluations?.length ? "—" : <AssignmentWeightInput value={kpi.weight} disabled={compositionReadOnly} onChange={(weight) => updateKpiWeight(kpi.id, weight)} />}</td></tr>{kpi.evaluations?.map(entity => <tr className="assignment-entity-evaluation" key={entity.subjectExternalId}><td/><td><strong>{entity.subjectLabel}</strong><small>Entity evaluation</small></td><td>Entity evaluation</td><td>{entity.goal ?? "—"} {entity.goalUnit?.symbol ?? kpi.goalUnit}</td><td>{entity.resultUnit?.symbol ?? kpi.resultUnit}</td><td/><td><input aria-label={`Weight for ${entity.subjectLabel}`} type="number" min="0" max="100" step="0.0001" placeholder="Required" value={entity.weight ?? ""} disabled={compositionReadOnly} onChange={event => { const weight = event.target.value === "" ? null : Number(event.target.value); setSaved(false); setKpis(items => items.map(item => item.id === kpi.id ? {...item, evaluations: item.evaluations?.map(value => value.subjectExternalId === entity.subjectExternalId ? {...value, weight} : value)} : item)); }} /> %</td></tr>)}</Fragment>)}
      </tbody></table></div>
    ...
    <footer className="assignment-actions">
      <div className="assignment-actions-content">
        ...
        <div className="assignment-composition-summary">
          ...
          <div className={`assignment-weight-track assignment-composition-track ${total > 100 ? "extended" : ""}`} role="progressbar" aria-label={`Weight breakdown: KPI ${kpiWeightTotal}%, Linked ScoreCards ${linkedWeightTotal}%, total ${total}%`} aria-valuemin={0} aria-valuemax={Math.max(100, total)} aria-valuenow={total}>
      ...
      <div className="assignment-primary-actions">
        {!compositionReadOnly && <span className="assignment-disabled-action" onClick={() => { if (saved && !saveMutation.isPending && !finalizeMutation.isPending) explainDisabledAction("Save Assignment is unavailable because there are no unsaved changes."); }}><button type="button" className="button primary" disabled={saved || saveMutation.isPending || finalizeMutation.isPending} onClick={save}><Save size={16} /> {saveMutation.isPending ? "Saving..." : "Save Assignment"}</button></span>}
        {!compositionReadOnly ? <span className="assignment-finalize-action" title={finalizeDisabledReason ?? "Finalize this Input Period composition."} onClick={() => { if (finalizeDisabledReason && !saveMutation.isPending && !finalizeMutation.isPending) explainDisabledAction(finalizeDisabledReason); }}><button type="button" className="button finalize-composition-button" disabled={Boolean(finalizeDisabledReason) || saveMutation.isPending || finalizeMutation.isPending} onClick={() => { setFinalizeAcknowledged(false); setFinalizeError([]); setFinalizeOpen(true); }}><Check size={16}/> Finalize Composition</button>{finalizeDisabledReason && <small>{finalizeDisabledReason}</small>}</span> : compositionQuery.data?.status === "FINALIZED" ? <span className="assignment-finalized-state"><Check size={16}/> Finalized</span> : null}
      </div>
    </footer>
    {saveNotice > 0 && <ActionToast key={saveNotice} message="ScoreCard assignment saved successfully." onClose={() => setSaveNotice(0)} />}
    {blockedActionMessage && <ActionToast message={blockedActionMessage} tone="info" onClose={() => setBlockedActionMessage("")} />}
