...
describe("Entity assignment weights",()=>{
  it("explains the Pool prerequisite and hides editing actions without requesting compositions",async()=>{
    ...
    open();
    expect(await screen.findByText("Pool Composition not finalized")).toBeInTheDocument();
    expect(scorecardService.composition).not.toHaveBeenCalled();
    expect(screen.queryByRole("button",{name:/Select KPIs from Pool/})).not.toBeInTheDocument();
    expect(screen.queryByRole("button",{name:/Save Assignment/})).not.toBeInTheDocument();
  });
  it("allows starting an empty assignment and loads previous scope for the period",async()=>{
    composition.kpis=[];
    ...
    open();
    expect(await screen.findByText("Select KPIs from this Pool to start the composition for this Input Period.")).toBeInTheDocument();
    await waitFor(()=>expect(scorecardService.composition).toHaveBeenCalledWith(1,"2026-08"));
    expect(scorecardService.composition).toHaveBeenCalledWith(1,"2026-09");
    expect(screen.getByText("Composition Weight")).toBeInTheDocument();
    expect(screen.getByRole("button",{name:/Select KPIs from Pool/})).toBeEnabled();
  });
