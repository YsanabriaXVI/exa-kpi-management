...
export async function importKpiConfigurationMocks(client: PrismaClient) {
  ...
  for (const item of initialKpiConfigurations) {
    ...
    if (item.status === "CONFIGURED") {
      const revision = await client.kpiConfigurationRevision.upsert({ where: { kpiConfigurationId_revisionNumber: { kpiConfigurationId: configuration.id, revisionNumber: 1 } }, update: { effectiveFrom: INITIAL_KPI_REVISION_EFFECTIVE_FROM, measurementUnitId: unit.id, dataSourceId: source.id }, create: { kpiConfigurationId: configuration.id, revisionNumber: 1, targetValue: item.goal, evaluationTypeId: evaluation.id, measurementUnitId: unit.id, dataSourceId: source.id, effectiveFrom: INITIAL_KPI_REVISION_EFFECTIVE_FROM, changeReason: "Imported from approved frontend development data" } });
