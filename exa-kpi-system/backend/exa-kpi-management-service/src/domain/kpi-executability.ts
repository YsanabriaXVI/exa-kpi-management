...
export type ExecutabilityInput = {
  ...
  entityEvaluationMode?: string | null;
...
const messages: Record<KpiExecutabilityReasonCode, string> = {
  SUM_REQUIRES_ADDITIVE_RESULT: "SUM requires additive counts or quantities in a supported Result Unit. Percentages, ratios, unit costs and durations are not additive.",
...
export function evaluateKpiExecutability(input: ExecutabilityInput): KpiExecutability {
  ...
  if (zero) {
    ...
    add(goals.some(goal => goal == null || Number(goal) !== 0) || !validResultBands(bands) || !bands.some(b => (b.minResult == null || b.minResult < 0 || b.minResult === 0 && b.includesMin !== false) && (b.maxResult == null || b.maxResult > 0 || b.maxResult === 0 && b.includesMax !== false)), "SCORING_RULE_INVALID");
  }
  ...
  const units = input.evaluationScope === "BY_SUBJECT" && input.entityEvaluationMode !== "CONTRIBUTE_TO_OVERALL" ? (input.subjectGoals ?? []).map(row => ({goal: row.goalUnit ?? input.goalUnit, result: row.resultUnit ?? input.measurementUnit})) : [{goal: input.goalUnit, result: input.measurementUnit}];
  ...
  if (input.evaluationScope === "OVERALL" || input.entityEvaluationMode === "CONTRIBUTE_TO_OVERALL") add(input.goal == null, "GOAL_REQUIRED");
  if (input.evaluationScope === "BY_SUBJECT") {
    ...
    add(input.entityEvaluationMode !== "CONTRIBUTE_TO_OVERALL" && ids.some((id) => goalById.get(id) == null), "SUBJECT_GOAL_REQUIRED");
  }
