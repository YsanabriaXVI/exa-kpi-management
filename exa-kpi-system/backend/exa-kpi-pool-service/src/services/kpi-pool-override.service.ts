...
export const kpiPoolOverrideService = {
  async resolve(poolId: bigint, inputPeriodId: bigint, configurationId: bigint, includeEditability = true) {
    ...
    if ((effective.evaluationScope === "OVERALL" || effective.entityEvaluationMode === "CONTRIBUTE_TO_OVERALL") && (effective.scoringMethod === "PROPORTIONAL" && !(Number(effective.goal) > 0) || effective.scoringMethod === "ZERO_TARGET_BANDS" && Number(effective.goal) !== 0)) effective.executability = { ...effective.executability, executable: false, status: "BLOCKED", reasons: [...effective.executability.reasons, { code: "POOL_GOAL_CONTRACT_INVALID", message: "The Pool goal must respect the scoring method confirmed in KPI Configuration" }] };
