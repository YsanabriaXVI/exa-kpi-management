...
export function live(_request: Request, response: Response): void {
  response.status(200).json({ status: "live", service: "exa-kpi-pool-service" });
}

export function ready(databaseCheck: DatabaseCheck) {
  return async (_request: Request, response: Response, _next: NextFunction): Promise<void> => {
    try {
      response.status(200).json(await getReadiness(databaseCheck));
    } catch {
      response.status(503).json({
        error: { code: "DATABASE_UNAVAILABLE", message: "The KPI Pool database is unavailable" },
      });
    }
  };
}
