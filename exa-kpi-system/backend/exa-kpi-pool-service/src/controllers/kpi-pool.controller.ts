...
import { addKpiPoolConfigurationsBodySchema, availableKpiConfigurationsQuerySchema, createKpiPoolBodySchema, extendKpiPoolValidityBodySchema, finalizePeriodCompositionBodySchema, globalConfigurationParamsSchema, kpiConfigurationUsageBodySchema, kpiPoolConfigurationParamsSchema, kpiPoolIdParamsSchema, listKpiPoolsQuerySchema, poolAssignmentEligibilityBodySchema, poolOverrideBodySchema, poolOverrideParamsSchema, replaceKpiPoolConfigurationBodySchema, resetPoolOverrideBodySchema, retireKpiPoolConfigurationBodySchema, targetPeriodQuerySchema, updateKpiPoolBodySchema } from "../schemas/kpi-pool.schema.js";
...
export async function getPoolAssignmentEligibility(request: Request, response: Response, next: NextFunction) { try { const { configurationIds } = poolAssignmentEligibilityBodySchema.parse(request.body); response.json(await kpiPoolMembershipService.assignmentEligibility(configurationIds)); } catch (error) { next(error); } }
export async function getEffectivePoolKpiSettings(request: Request, response: Response, next: NextFunction) { try { const { id, inputPeriodId, configurationId } = poolOverrideParamsSchema.parse(request.params); response.json({ data: await kpiPoolOverrideService.resolve(BigInt(id), BigInt(inputPeriodId), BigInt(configurationId)) }); } catch (error) { next(error); } }
export async function savePoolKpiOverride(request: Request, response: Response, next: NextFunction) { try { const { id, inputPeriodId, configurationId } = poolOverrideParamsSchema.parse(request.params); response.json({ data: await kpiPoolOverrideService.save(BigInt(id), BigInt(inputPeriodId), BigInt(configurationId), poolOverrideBodySchema.parse(request.body), request.identity.actorUserId) }); } catch (error) { next(error); } }
export async function resetPoolKpiOverride(request: Request, response: Response, next: NextFunction) { try { const { id, inputPeriodId, configurationId } = poolOverrideParamsSchema.parse(request.params); response.json({ data: await kpiPoolOverrideService.reset(BigInt(id), BigInt(inputPeriodId), BigInt(configurationId), resetPoolOverrideBodySchema.parse(request.body), request.identity.actorUserId) }); } catch (error) { next(error); } }
export async function listEligibleGlobalEditPeriods(request: Request, response: Response, next: NextFunction) { try { const { configurationId } = globalConfigurationParamsSchema.parse(request.params); response.json(await kpiPoolOverrideService.eligibleGlobalPeriods(BigInt(configurationId))); } catch (error) { next(error); } }
