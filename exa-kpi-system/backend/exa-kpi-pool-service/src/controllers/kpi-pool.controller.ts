...
export async function getEffectivePoolKpiSettings(request: Request, response: Response, next: NextFunction) { try { const { id, inputPeriodId, configurationId } = poolOverrideParamsSchema.parse(request.params); response.json({ data: await kpiPoolOverrideService.resolve(BigInt(id), BigInt(inputPeriodId), BigInt(configurationId), request.query.preview !== "true") }); } catch (error) { next(error); } }
...
export async function removeKpiPool(request: Request, response: Response, next: NextFunction) { try { const { id } = kpiPoolIdParamsSchema.parse(request.params); response.json({ data: await kpiPoolLifecycleService.remove(BigInt(id), request.identity.actorUserId) }); } catch (error) { next(error); } }
