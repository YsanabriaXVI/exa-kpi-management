...
The normal operational activation path is finalizing the first Pool composition. That command revalidates the first period membership and atomically persists its immutable composition, changes lifecycle `DRAFT → ACTIVE`, and writes the existing activation plus composition-finalized Outbox events. The explicit `POST /api/v1/kpi-pools/:id/activate` command remains available for compatibility, but UI workflow must not activate merely because KPIs were added.

While DRAFT, structural information and the initial membership may change freely. Once ACTIVE, structural identity remains locked: areas, companies, `valid_from`, frequency, issue year and code do not change. Only the descriptive `pool_name` and `description` fields remain editable; `notes` and all other structural fields stay locked. `valid_to` is the sole controlled structural exception: `POST /api/v1/kpi-pools/:id/extend-validity` may move it later, never earlier, while preserving complete consecutive periods, the original frequency and the total 12-month validity limit. Existing periods and finalized snapshots are not rewritten. New Input Periods are deterministically derived from the extended boundary, and `kpi.pool.validity.extended.v1` records the old/new end plus added periods. `ACTIVE` means published for Scorecard use; it no longer freezes KPI membership forever. Membership additions, retirements and replacements may be scheduled only for the one currently eligible composition. Later periods remain `FUTURE` and expose neither an editable composition nor a projected KPI preview. INACTIVE accepts no new planning and never destroys history.
...
Pool now owns the narrower **Pool Composition per Input Period** decision. `POST /api/v1/kpi-pools/:id/input-periods/finalize` accepts either the first eligible composition of a DRAFT Pool or a dependency-approved composition of an ACTIVE Pool. It validates period boundaries, at least one effective membership, unique Definitions, active Configuration/Definition/frequency references and exact frequency compatibility. It then persists `EDITABLE → POOL_COMPOSITION_LOCKED` in `kpi_pool_period_compositions` and writes `kpi.pool.period.composition.finalized.v1` to Outbox in the same transaction. First-period finalization additionally activates the Pool and locks structural metadata. This does not publish Scorecards, close Monitoring, close the Pool, or create a Monitoring result period. The three concepts remain distinct: `POOL_COMPOSITION_LOCKED`, future `SCORECARDS_PUBLISHED`, and future `MONITORING_CLOSED`. A finalized composition permanently rejects add, retire and replace commands for that period; the next eligible period remains independently editable.
...
Only one not-yet-finalized Input Period is operationally editable at a time. For example, while August's finalized composition is being used downstream, September may be prepared but October and later periods remain `FUTURE`. September finalization still waits for August Monitoring closure. Eligibility for the next editable composition is derived centrally and deterministically from persisted Pool composition locks and the downstream closure contract; the UI must not manufacture inherited or projected compositions.
...
## Period workflow and UI responsibilities

The cross-context workflow is `Pool Composition → Scorecard KPI Selection → Monitoring Results`, with a different owner at each stage:

- KPI Management owns how a KPI is defined and measured.
- KPI Pool owns which KPI Configurations are made available for one Input Period.
- Scorecards own which available KPI Configurations are selected and the weight assigned to them.
- Monitoring owns input/result solicitation, validation, exceptions and closure.

A finalized Pool composition is the immutable set of KPI Configurations made available to Scorecards for exactly one Input Period. It does not select KPIs for a Scorecard, publish a Scorecard, generate Monitoring inputs or start Monitoring. Expected Monitoring inputs come from the KPI Configurations actually selected by applicable Scorecards for that period, never automatically from the full Pool composition.

Finalized compositions are read-only. A future correction before results would require an explicit `Reopen Composition` command and authoritative Scorecard plus Monitoring dependency checks; no such command exists yet. Once results exist, historical composition must never be changed retroactively. Absence of Monitoring results alone is insufficient permission because Scorecards may already reference a finalized KPI Configuration.

The Pool UI keeps three responsibilities separate. **Pool Detail** is read-oriented and shows Pool identity plus effective or finalized composition snapshots by period. **Manage KPIs** edits only the currently eligible composition and finalizes that composition when the backend permits it. **Pool Period Schedule** is a read-only workflow map and navigation surface; it may show Pool-owned state but must label Scorecard or Monitoring state unavailable until a real downstream contract supplies it.

Pool does not own Scorecard selections or Monitoring Results and must not reproduce either state machine. Future integration may use synchronous REST for finalization validation or a NATS-fed local projection for asynchronous visibility. The backend remains authoritative and must reject invalid finalization even when the frontend disables the action.

### Published finalization contract

`kpi.pool.period.composition.finalized.v1` is the authoritative fact that makes one Pool set available downstream. Its data contains stable `poolPeriodId`, `poolCompositionId`, `periodKey`, period boundaries, KPI count, and the finalized membership snapshot. Every membership supplies `poolMembershipId`, `kpiDefinitionId`, `kpiConfigurationId`, code/name snapshots, and display order.

`kpi_pool_input_periods` persists the Input Period identity generated from validity and frequency. Rows are created with the Pool and extended transactionally with validity; this is not a second editable calendar. Consumers store these values as External IDs and never create cross-database FKs to Pool tables.
