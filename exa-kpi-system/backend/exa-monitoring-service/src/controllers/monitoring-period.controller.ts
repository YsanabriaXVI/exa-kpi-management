...
import { exceptionWorkflowBodySchema } from "../schemas/monitoring-period.schema.js";
...
export async function approveMonitoringPeriod(request:Request,response:Response,next:NextFunction){try{const{id}=monitoringPeriodIdParamsSchema.parse(request.params);response.json(await monitoringWorkflowService.approve(id,exceptionWorkflowBodySchema.parse(request.body),request.identity.actorUserId));}catch(error){next(error);}}
