...
const schema = z.object({
  MONITORING_RECONCILIATION_ENABLED:z.string().default("true").transform(value=>value==="true"),
  MONITORING_RECONCILIATION_INTERVAL_MS:z.coerce.number().int().min(1000).default(30000),
  ...
  KPI_POOL_BASE_URL: z.string().url().default("http://localhost:4002"), SCORECARDS_BASE_URL: z.string().url().default("http://localhost:4003"), SERVICE_HTTP_TIMEOUT_MS: z.coerce.number().int().positive().max(30000).default(5000),
