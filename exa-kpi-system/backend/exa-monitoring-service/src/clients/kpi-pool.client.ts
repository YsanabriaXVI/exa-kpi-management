...
export const kpiPoolClient={
  async list(){
    const pools:Pool[]=[];let page=1,totalPages=1;
    do { const response=await serviceRequest<{data:Pool[];meta:{totalPages:number}}>("KPI Pool",env.KPI_POOL_BASE_URL,`/api/v1/kpi-pools?page=${page}&pageSize=100`);pools.push(...response.data);totalPages=response.meta.totalPages;page++; } while(page<=totalPages);
    return pools;
  },
