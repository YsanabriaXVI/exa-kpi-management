...
export const scorecardService = {
  ...
  async create(input: CreateScorecardBody, actor: bigint) {
    const pool = await draftPool(input.kpiPoolExternalId); validateScope(input, pool);
  ...
  async update(id: bigint, input: UpdateScorecardBody, actor: bigint) {
    ...
    if (pool.status !== "DRAFT" && (input.departments !== undefined || input.collaborators !== undefined)) throw new AppError(409, "SCORECARD_STRUCTURE_LOCKED", "Scorecard structural scope cannot change after the KPI Pool leaves DRAFT");
