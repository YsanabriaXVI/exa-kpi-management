...
function dto(value: ScorecardRow, pool?: { validFrom: Date; validTo: Date; inputFrequencyCode: string } | null, poolPeriods: PoolPeriodRow[] = []) {
  return { id: value.id.toString(), code: value.code, name: value.name, description: value.description, status: value.statusCode, kpiPool: { id: value.kpiPoolExternalId.toString(), code: value.poolCodeSnapshot, name: value.poolNameSnapshot }, poolSchedule: pool ? { validFrom: dateOnly(pool.validFrom), validTo: dateOnly(pool.validTo), inputFrequencyCode: pool.inputFrequencyCode, inputPeriods: periodCount(pool.validFrom, pool.validTo, pool.inputFrequencyCode) } : null, currentComposition: currentComposition(value, poolPeriods), companies: value.companies.map((item) => ({ id: item.externalCompanyId.toString(), code: item.companyCodeSnapshot, name: item.companyNameSnapshot })), departments: value.departments.map((item) => ({ id: item.externalDepartmentId.toString(), companyId: item.externalCompanyId.toString(), code: item.departmentCodeSnapshot, name: item.departmentNameSnapshot, collaborators: item.employees.map((employee) => ({ id: employee.externalEmployeeId.toString(), code: employee.employeeCodeSnapshot, name: employee.employeeNameSnapshot })) })), periodCompositionCount: value.periodCompositions.length, createdAt: value.createdAt.toISOString(), updatedAt: value.updatedAt?.toISOString() ?? null, createdByUserId: value.createdByUserId?.toString() ?? null, updatedByUserId: value.updatedByUserId?.toString() ?? null };
}
function validateScope(input: Pick<CreateScorecardBody, "departments" | "collaborators">, pool: PoolRecord) {
  if (!input.collaborators.length) throw new AppError(422, "SCORECARD_COLLABORATOR_REQUIRED", "At least one Collaborator must be assigned to the Scorecard");
...
async function draftPool(id: string) { const pool = await kpiPoolClient.getPool(id); if (pool.status !== "DRAFT") throw new AppError(409, "KPI_POOL_NOT_DRAFT", "The selected KPI Pool must be DRAFT"); return pool; }
async function scorecardSourcePool(id: string) {
...
export const scorecardService = {
  ...
  async create(input: CreateScorecardBody, actor: bigint) {
    const pool = await scorecardSourcePool(input.kpiPoolExternalId); validateScope(input, pool);
  ...
  async update(id: bigint, input: UpdateScorecardBody, actor: bigint) {
    ...
    const pool = await kpiPoolClient.getPool(current.kpiPoolExternalId.toString());
    if (current.statusCode !== "DRAFT" && (input.departments !== undefined || input.collaborators !== undefined)) throw new AppError(409, "SCORECARD_STRUCTURE_LOCKED", "Scorecard structural scope cannot change after its first composition is finalized");
  ...
  async deactivate(id: bigint, actor: bigint) { const current = await prisma.scorecard.findFirst({ where: { id, deletedAt: null } }); if (!current) throw new AppError(404, "SCORECARD_NOT_FOUND", "Scorecard was not found"); await draftPool(current.kpiPoolExternalId.toString()); return prisma.$transaction(async (tx) => { const updated = await tx.scorecard.updateMany({ where: { id, deletedAt: null, statusCode: { in: ["DRAFT", "ACTIVE"] } }, data: { statusCode: "INACTIVE", aggregateVersion: { increment: 1 }, updatedAt: new Date(), updatedByUserId: actor } }); if (!updated.count) throw new AppError(409, "SCORECARD_DEACTIVATION_CONFLICT", "Scorecard is missing or already INACTIVE"); const row = await tx.scorecard.findUniqueOrThrow({ where: { id }, include }); await outbox(tx, row, "scorecard.deactivated.v1", { scorecardCode: row.code }); return dto(row); }); },
  async remove(id: bigint, actor: bigint) { const current = await prisma.scorecard.findFirst({ where: { id, deletedAt: null } }); if (!current) throw new AppError(404, "SCORECARD_NOT_FOUND", "Scorecard was not found"); return prisma.$transaction(async (tx) => { const updated = await tx.scorecard.updateMany({ where: { id, deletedAt: null }, data: { deletedAt: new Date(), statusCode: "INACTIVE", aggregateVersion: { increment: 1 }, updatedAt: new Date(), updatedByUserId: actor } }); if (!updated.count) throw new AppError(409, "SCORECARD_DEACTIVATION_CONFLICT", "Scorecard is missing or already removed"); const row = await tx.scorecard.findUniqueOrThrow({ where: { id }, include }); await outbox(tx, row, "scorecard.deactivated.v1", { scorecardCode: row.code }); return dto(row); }); },
};
