...
describe("Scorecard Information service", () => {
  ...
  it("rejects an expired Pool as Scorecard source", async () => { poolClient.getPool.mockResolvedValue({ ...pool, validTo: "2020-12-31" }); await expect(scorecardService.create(input, 7n)).rejects.toMatchObject({ code: "KPI_POOL_EXPIRED" }); expect(db.$transaction).not.toHaveBeenCalled(); });
