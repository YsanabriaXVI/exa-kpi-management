...
const pool = { id: "2", poolCode: "OPS-01-2026", poolName: "Operations", status: "DRAFT", issueYear: 2026, validFrom: "2026-08-01", validTo: "2026-12-31", inputFrequency: { id: "1", code: "MONTHLY" }, companies: [{ id: "1", code: "EXA", name: "EXA" }], areas: [{ id: "1", code: "OPS", name: "Operations" }] };
...
describe("Scorecard Information service", () => {
  ...
  it("rejects creation after the Pool leaves DRAFT", async () => { poolClient.getPool.mockResolvedValue({ ...pool, status: "ACTIVE" }); await expect(scorecardService.create(input, 7n)).rejects.toMatchObject({ code: "KPI_POOL_NOT_DRAFT" }); expect(db.$transaction).not.toHaveBeenCalled(); });
